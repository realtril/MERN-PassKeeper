...
export const passwordUpdateOperation = updatedPassword => async (
  dispatch,
  getState,
) => {
  dispatch(loaderOn());
  ...
  magicToken.set(token);
  try {
    const res = await axios.patch(`/passwords/${passwordId}`, {
      ...updatedPassword,
    });
    dispatch(
      passActions.passwordChangeSuccess({ updatedPassword, passwordId }),
    );
  } catch (error) {
